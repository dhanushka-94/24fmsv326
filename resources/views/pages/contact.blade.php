...
<x-page-shell>
    ...
    <main class="mx-auto max-w-7xl px-4 pb-20 pt-16 sm:px-6 lg:px-12 lg:pt-20">
        <section class="grid gap-10 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
            <form action="{{ route('contact.store') }}" method="POST" class="reveal card-surface space-y-4 p-6 text-sm text-body sm:p-8">
                @csrf
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="space-y-1.5">
                        ...
                        @error('name')<p class="text-xs text-accent-strong">{{ $message }}</p>@enderror
                    </div>
                    <div class="space-y-1.5">
                        ...
                        @error('email')<p class="text-xs text-accent-strong">{{ $message }}</p>@enderror
                    </div>
                </div>
                <div class="space-y-1.5">
                    <label class="section-label !tracking-[0.22em]">Project details</label>
                    <textarea name="message" rows="5" required class="field-input min-h-[8rem] py-2" placeholder="Tell us about your production, timelines, and locations.">{{ old('message') }}</textarea>
                    @error('message')<p class="text-xs text-accent-strong">{{ $message }}</p>@enderror
                </div>
                ...
                @if (session('status'))
                    <p class="text-xs text-accent">{{ session('status') }}</p>
                @endif
            </form>
